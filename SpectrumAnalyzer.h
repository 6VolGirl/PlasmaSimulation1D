...
private:
    ...
    std::vector<std::complex<double>> poynting_f;     // спектр S(f)

    std::vector<std::complex<double>> fftRecursive(
...
//         }
//
//         // Берём уже посчитанный S(t) = E * H_interp из Monitor
//         poynting_f = computeFFT(m.poynting, dt);
//
//     }
//
//     void writeSpectrumCSV(const std::string& filename, double fL) const {
